...
// The judge-behavior switches: repairing an unparsable analysis is on because
// the alternative is losing the analysis, while the two that change what the
// caller observes are opt-in.
func TestResolveFusionExecutionConfigJudgeControlDefaults(t *testing.T) {
	looper := NewFusionLooper(&config.LooperConfig{Endpoint: "http://looper"})
	req := newFusionTestRequest()
	req.Algorithm = &config.AlgorithmConfig{
		Type:   "fusion",
		Fusion: &config.FusionAlgorithmConfig{Model: "judge", AnalysisModels: []string{"panel-a"}},
	}

	cfg := looper.resolveFusionExecutionConfig(req)

	assert.True(t, cfg.AnalysisParseRetry)
	assert.False(t, cfg.SkipAnalysisOnAgreement)
	assert.False(t, cfg.AgenticJudgeRules)
}

func TestResolveFusionExecutionConfigJudgeControlsOverrideFromRequest(t *testing.T) {
	looper := NewFusionLooper(&config.LooperConfig{Endpoint: "http://looper"})
	req := newFusionTestRequest()
	req.Algorithm = &config.AlgorithmConfig{
		Type: "fusion",
		Fusion: &config.FusionAlgorithmConfig{
			...
			AnalysisParseRetry: boolPtr(false),
			AgenticJudgeRules:  boolPtr(true),
		},
	}
	req.Fusion = &config.FusionRequestConfig{ID: "fusion", SkipAnalysisOnAgreement: boolPtr(true)}

	cfg := looper.resolveFusionExecutionConfig(req)

	assert.False(t, cfg.AnalysisParseRetry)
	assert.True(t, cfg.SkipAnalysisOnAgreement)
	assert.True(t, cfg.AgenticJudgeRules)
}
